...
function LightControl(props: LightInfo) {
  ...
  const handleToggleLight = useCallback(
    (deviceIds: string[], status: boolean) => {
      ...
      const isSTLSmart = props.device.type === 'lms.devices.types.STL_SMART';

      // If STL Smart, also update brightness state locally and call API
      if (isSTLSmart) {
        const brightness = status ? 100 : 0;
        setBrightnessMap((prev) => {
          const updated = { ...prev };
          deviceIds.forEach((id) => (updated[id] = brightness));
          return updated;
        });
        ...
        return; // Dùng lệnh gộp nên thoát sớm
      }
